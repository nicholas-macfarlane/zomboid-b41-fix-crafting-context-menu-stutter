...
function ISRecipeTooltip.layoutContents(self, x, y)
	...
	for _,source in ipairs(allSources) do
		...
		for k=1,source:getItems():size() do
            ...
            --only add items available to the player, not the entire crafting library
            if itemData.available then
			    table.insert(itemDataList, itemData)
            end
		end
		--and now we don't need to sort through the entire recipe library to see what's available.
